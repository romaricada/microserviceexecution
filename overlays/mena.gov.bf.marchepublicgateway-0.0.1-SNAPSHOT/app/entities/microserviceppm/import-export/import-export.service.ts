...
export class ImportExportService {
  ...
  importDataPPMDeconcentre(id: number, file: File, reference: string, update: boolean): Observable<HttpResponse<any>> {
    const formData = new FormData();
    formData.append('file', file, file.name);
    let header = new HttpHeaders();
    header = header.append('Accept', 'application/vnd.ms.excel; charset=utf-8');
    const options = createRequestOption({ id, reference, update });
    return this.http.post<string>('services/microserviceppm/api/import-ppm-deconcentre-data', formData, {
      headers: header,
      params: options,
      observe: 'response'
    });
  ...
  importDataBudget(id: number, uniteAdministrativeId: number, file: File, update: boolean): Observable<HttpResponse<any>> {
    const formData = new FormData();
    formData.append('file', file, file.name);
    let header = new HttpHeaders();
    header = header.append('Accept', 'application/vnd.ms.excel; charset=utf-8');
    const options = createRequestOption({ id, uniteAdministrativeId, update });
